...
class C_PerangkatDesa extends CI_Controller {
	...
	public function tambah_data()
	{
		...
		$data['main_view']='desa/V_tambah_data';
		$this->load->view('admin/index',$data);
	...
	public function tambah_perangkat()
	{
		...
		if ($this->form_validation->run() ==  FALSE ) {
			...
			redirect('C_PerangkatDesa/tambah_data');
	...
	public function edit_perangkat()
	{
		...
		$data['perangkat']=$this->db->get('perangkat_desa')->result();
		$data['main_view']='desa/V_edit_perangkat';
		$this->load->view('admin/index', $data);
	...
	public function cetak_data()
	{
		...
		$id=$this->session->userdata('id_desa');
		$query=$this->db->query("SELECT * FROM desa  INNER JOIN kabupaten ON desa.id_kab=kabupaten.id_kab INNER JOIN kecamatan ON desa.id_kec=kecamatan.id_kec  WHERE id_desa='$id'")->row_array();
        ...
        $pdf->MultiCell(14, 0.5, "Desa         : ".$query['nama_desa'], 0, 'L');
	...
	public function daftar_kades()
	{
		$id=$this->session->userdata('id_desa');
		$data['kades']=$this->db->query("SELECT * FROM history_aparatur WHERE id_desa='$id'")->result();
		$data['main_view']='desa/V_daftar_kades';
		$this->load->view('admin/index', $data);
	}

	public function send()
	{
		$data['send']=$this->db->where('id_desa',$this->session->userdata('id_desa'));
